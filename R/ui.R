...
ui <- function() {
  (fluidPage(
    ...
    sidebarLayout(
      sidebarPanel(
        conditionalPanel(condition = "input.tabs1!=='14C Calibration'",
      ...
      fluidRow(
        ...
        conditionalPanel(condition = "input.tabs1=='14C Calibration'",
                         ...
                         fluidRow(
                           column(
                             12,
                             uiOutput("ui.col.select.14C")
                           )
                         ),
                         fluidRow(
                           column(
                             12,
                             uiOutput("ui.14C.pars")
                           )
                         ),
                         fluidRow(
                           column(
                             12,
                             uiOutput("ui.start.14C")
                           )
                         ),
                         fluidRow(
                           uiOutput("ui.download.cal")
                         )
                         ),
        conditionalPanel(condition = "input.tabs1!=='14C Calibration'",
      ...
      mainPanel(
        ...
        tabsetPanel(
          ...
          # 14C Calibration panel ------
          tabPanel("14C Calibration",
                   tags$head(
                     tags$style("#datafile.14C{overflow-y:scroll; max-height: 400px; min-height: 200px;}")
                   ),
                   #fluidRow(tableOutput("datafile")),
                   fluidRow(tableOutput("datafile.14C"))
                   ),
          # modelling panel --------
          tabPanel(
            "Model",
